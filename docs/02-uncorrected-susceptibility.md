...
3. **$\hat{H}$ and its sampling distribution.** We form $$\hat{H} = \hat{\sigma}_f^2 \cdot \hat{\sigma}_{r}^2$$ and use a weighted block jackknife over approximately independent LD blocks to obtain a standard error and a $p$-value against the null $H \approx 1/L$.
